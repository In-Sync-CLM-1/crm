...
**Engine Status**: **LIVE** — 12 pg_cron jobs active, all edge functions deployed, frontend deployed to Cloudflare Pages
...
| System | `mkt_engine_config`, `mkt_engine_logs`, `mkt_unsubscribes`, `mkt_exit_surveys`, `mkt_client_outcomes` |
...
| Channels | 5 per org | email, whatsapp, google_ads, meta_ads, linkedin (auto-seeded; voice channel pending Bolna integration) |
...
- **M3** (10 clients): Voice calls (Bolna), NPS engine
...
| `GROQ_API_KEY` | Configured | Feature signal extraction, real-time call inference |
...
| Key | When Needed |
|-----|-------------|
| `BOLNA_API_KEY` | When testing AI voice calls (Bolna integration) |
...
|---------|-------------|-------------|
| Resend | `https://mlvgqudcwlkolsbighnn.supabase.co/functions/v1/mkt-email-webhook` | Registered via Resend API |
| Exotel WhatsApp | `https://mlvgqudcwlkolsbighnn.supabase.co/functions/v1/mkt-whatsapp-webhook` | Set per-message via `status_callback` |
| Product Events | `https://mlvgqudcwlkolsbighnn.supabase.co/functions/v1/mkt-product-webhook` | Called by external products |
...
| Voice calls (Bolna) | Pending | Migrating from VAPI; Exotel App-Bazaar Voicebot flow being configured |
...
6. **Webhook registration**: Resend webhook registered via API; Exotel sets callback URLs per-message
