...
import { Mail, Phone as PhoneIcon, Building, LayoutGrid, Table as TableIcon, Loader2, Phone, MapPin, Factory, MessageSquare, MoreHorizontal, Pencil, Trash2, UserPlus, MessageCircle, Plus, PhoneCall, X } from "lucide-react";
import { Checkbox } from "@/components/ui/checkbox";
import { BulkAICallDialog } from "@/components/Pipeline/BulkAICallDialog";
  ...
  return (
    <DashboardLayout>
      <div className="space-y-4">
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="py-2 text-xs h-8 w-8">
                        <Checkbox
                  ...
                  <TableBody>
                    {contacts.map((contact, index) => {
                      ...
                      return (
                        <TableRow
                          key={contact.id}
                          className={`hover:bg-muted/50 ${index % 2 === 0 ? 'bg-muted/20' : ''} ${selectedIds.has(contact.id) ? 'bg-primary/5' : ''}`}
                        >
                          <TableCell className="py-1.5 w-8" onClick={(e) => e.stopPropagation()}>
                            <Checkbox
        ...
        {/* Bulk-action sticky bar (visible when any contact is selected) */}
        {selectedIds.size > 0 && (
          <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-background border shadow-lg rounded-full px-4 py-2 flex items-center gap-3">
            ...
            <Button size="sm" onClick={() => setBulkCallOpen(true)} className="gap-1">
