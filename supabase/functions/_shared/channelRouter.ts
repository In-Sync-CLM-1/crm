...
export async function getNextChannel(
  ...
  // If the call channel is not active, treat 'call' steps as if the channel
  // is unavailable and fall through immediately to the fallback chain (whatsapp → email).
  let effectiveChannel = step.channel;
  if (step.channel === 'call') {
    const { data: callChannel } = await supabase
      .from('mkt_channels')
      .select('active')
      .eq('channel_key', 'call')
      .maybeSingle();
    if (!callChannel?.active) {
