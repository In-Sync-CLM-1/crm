...
/**
 * Call Cerebras (OpenAI-compatible API) — the 'haiku' tier's primary provider.
 * Falls back to Groq if Cerebras is unavailable or errors out.
 */
async function callCerebras(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        // Rate limit / overloaded — wait and retry (cap at 8s to avoid function timeout)
  ...
  throw lastError || new Error('Cerebras call failed after retries');
}

/**
 * Unified LLM caller. The 'haiku' tier runs on Cerebras (falls back to Groq on
 * failure); the 'sonnet' tier runs on Claude Sonnet via Anthropic (falls back
 * to Groq automatically if Anthropic credits are exhausted).
 */
