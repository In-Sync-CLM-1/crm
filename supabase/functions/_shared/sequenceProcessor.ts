...
export async function processEnrollment(
  ...
  // If the step is 'call' but the call channel isn't active, the router falls
  // back to email/WA with no appropriate template. Skip and advance rather than fail.
  if ((step.channel as string) === 'call' && channelResult.channel !== 'call') {
    await supabase
      .from('mkt_sequence_actions')
      .update({ status: 'skipped', failure_reason: 'Call step skipped — call channel not active' })
      .eq('id', action!.id);
