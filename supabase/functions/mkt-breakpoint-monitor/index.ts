...
const BREAKPOINT_DEFS: BreakpointDef[] = [
  ...
  {
    name: 'Call Complaints',
    category: 'operational',
    consecutiveWeeks: 1,
    afterMonth3: false,
    pausedComponent: 'voice-calls',
    check: (_rows, ctx) => {
...
async function processOrg(
  ...
  for (const def of BREAKPOINT_DEFS) {
    ...
    if (isTriggered && !isAlreadyActive) {
      ...
      // --- Campaign auto-pause DISABLED (2026-05-27) ---
      // Requirement: campaigns must stay active every day. The engine no longer
      // pauses campaigns on breakpoint triggers. We still DETECT + ALERT so the
      // signal isn't lost, but campaign status is never changed here.
      const pausedCount = 0;
      await supabase.from('mkt_engine_logs').insert({
        org_id: orgId,
        function_name: 'mkt-breakpoint-monitor',
        action: 'breakpoint-detected-no-pause',
        level: 'warn',
        details: {
          breakpoint: def.name,
          threshold: result!.data.threshold || def.pausedComponent,
          note: 'auto-pause disabled by design; campaign left active',
        },
      });
