...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const orgId of orgIds) {
      ...
      // Auto-pause DISABLED (2026-05-27): campaigns must stay active every day.
      // We keep the 'pause' recommendation for visibility, but never apply it.
      for (const rec of recommendations) {
        if (rec.type === 'pause' && rec.priority === 'high') {
          rec.auto_applied = false;

          await logger.warn('pause-recommendation-not-applied', {
