    ...
    for (const orgId of orgIds) {
      try {
        ...
        const { error: insertError } = await supabase.from('mkt_engine_metrics').insert({
          ...
          call_answer_rate: metrics.call_answer_rate,
          call_positive_rate: metrics.call_positive_rate,
...
async function collectOrgMetrics(
  ...
  return {
    ...
    call_answer_rate: channelMetrics.call_answer_rate,
    call_positive_rate: channelMetrics.call_positive_rate,
...
async function collectChannelMetrics(
  ...
  return {
    ...
    call_answer_rate: callsMade > 0 ? round4(callsAnswered / callsMade) : null,
    call_positive_rate: callsMade > 0 ? round4(callsPositive / callsMade) : null,
