...
// Cap: delivered/day PER CAMPAIGN — each campaign gets its own independent budget.
// 2026-05-27: set to 100 for ALL products (cleared worksync/vendorverification
// 1000 overrides) per request — every product's new-contact daily outreach = 100.
const BATCH_SIZE = 25;
const DEFAULT_DAILY_OUTREACH_LIMIT = 100;
const PRODUCT_DAILY_LIMITS: Record<string, number> = {};

function dailyLimitFor(productKey: string | null | undefined): number {
  if (productKey && PRODUCT_DAILY_LIMITS[productKey]) return PRODUCT_DAILY_LIMITS[productKey];
  return DEFAULT_DAILY_OUTREACH_LIMIT;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Send window: 10:00–18:00 IST only. The cron still runs every 5 min, but
    // campaigns only fire during business hours (UTC+5:30). Outside it, no-op.
    const istMinutes = (() => {
    ...
    if (istMinutes < 600 || istMinutes >= 1080) {
      return new Response(
        JSON.stringify({ message: 'Outside send window (10:00-18:00 IST)', executed: 0 }),
    ...
    for (const campaign of activeCampaigns) {
      ...
      const dailyLimit = dailyLimitFor(campaign.product_key as string | null);
      const safetyMax = Math.floor(dailyLimit * MAX_SEND_MULTIPLIER);

      const remaining = delivered >= dailyLimit || totalSent >= safetyMax
        ? 0
        : Math.max(0, dailyLimit - totalSent);
