...
SELECT cron.schedule('linkedin-slot-0', '0 2 * * *',    -- 07:30 IST
  $$SELECT net.http_post(url:='https://mlvgqudcwlkolsbighnn.supabase.co/functions/v1/mkt-blog-writer',headers:='{"Content-Type":"application/json","Authorization":"Bearer ***REDACTED_JWT***"}'::jsonb,body:='{}'::jsonb) AS request_id$$);
...
SELECT cron.schedule('linkedin-engagement-tracker', '30 17 * * *',  -- 23:00 IST
  $$SELECT net.http_post(url:='https://mlvgqudcwlkolsbighnn.supabase.co/functions/v1/mkt-linkedin-engagement-tracker',headers:='{"Content-Type":"application/json","Authorization":"Bearer ***REDACTED_JWT***"}'::jsonb,body:='{}'::jsonb) AS request_id$$);
