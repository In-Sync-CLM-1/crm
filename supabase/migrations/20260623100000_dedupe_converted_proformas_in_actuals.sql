-- Fix revenue double-count: a proforma that has been converted into a tax
-- invoice was still being summed alongside that invoice in the monthly
-- actuals (the chart + Target-vs-Achievement table). The proforma and the
-- invoice it became are the same money, so the proforma must drop out once an
-- active invoice references it via converted_from_id.
--
--   Invoiced branch: drop a proforma if any invoice converted from it is
--                    active (status NOT IN draft/cancelled).
--   Received branch: drop a proforma if any invoice converted from it is
--                    itself paid (so it is the one counted as received).

CREATE OR REPLACE FUNCTION public.get_monthly_actuals_optimized(_org_id uuid, _year integer)
 RETURNS TABLE(month integer, qualified bigint, proposals bigint, deals bigint, invoiced numeric, received numeric, invoiced_invoice_ids uuid[], received_invoice_ids uuid[], qualified_contact_ids uuid[], proposal_contact_ids uuid[], deal_contact_ids uuid[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  all_invoiced_rows AS (
    ...
    UNION ALL
    SELECT
      EXTRACT(MONTH FROM bd.doc_date::date)::integer AS m,
      bd.total_amount AS total,
      bd.id
    FROM billing_documents bd
    WHERE bd.org_id = _org_id
      AND EXTRACT(YEAR FROM bd.doc_date::date) = _year
      AND bd.doc_type IN ('invoice', 'proforma')
      AND bd.status NOT IN ('draft', 'cancelled')
      -- Drop a proforma once it has been converted into an active invoice
      AND NOT (
        bd.doc_type = 'proforma'
        AND EXISTS (
          SELECT 1 FROM billing_documents bi
          WHERE bi.converted_from_id = bd.id
            AND bi.doc_type = 'invoice'
            AND bi.status NOT IN ('draft', 'cancelled')
        )
      )
  ),
  invoiced_data AS (
  ...
  all_received_rows AS (
    ...
    SELECT
      EXTRACT(MONTH FROM bd.doc_date::date)::integer AS m,
      COALESCE(bd.amount_paid, bd.total_amount) AS total,
      bd.id
    FROM billing_documents bd
    WHERE bd.org_id = _org_id
      AND EXTRACT(YEAR FROM bd.doc_date::date) = _year
      AND bd.doc_type IN ('invoice', 'proforma')
      AND bd.status = 'paid'
      -- Drop a paid proforma once it has been converted into a paid invoice
      AND NOT (
        bd.doc_type = 'proforma'
        AND EXISTS (
          SELECT 1 FROM billing_documents bi
